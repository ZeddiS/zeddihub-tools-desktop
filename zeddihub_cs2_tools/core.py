...
def setup_console():
    if platform.system() == "Windows":
        try:
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(f"ZeddiHub CS2 Tools {VERSION}")
...
def print_header():
    clear_console()
    logo = [
        r"    _____ _____ ___    _______          _     ",
        r"   / ____/ ____|__ \  |__   __|        | |    ",
        r"  | |   | (___    ) |    | | ___   ___ | |___ ",
        r"  | |    \___ \  / /     | |/ _ \ / _ \| / __|",
        r"  | |___ ____) |/ /_     | | (_) | (_) | \__ \\",
        r"   \_____|_____/|____|   |_|\___/ \___/|_|___/"
    ]
    for line in logo:
        print(MAGENTA + center(line) + RESET)
...
def render_menu(title, options, sel, descs=None, centered=False, footer=None, extra_art=None):
    ...
    if extra_art:
        print()
        for line in extra_art: print(center(line))
    print("\n" + GRAY + center("-" * 80) + RESET)
...
def multi_select_menu(title, options, selected_flags, descs=None):
    sel = 0
    while True:
        display_opts = [f"{'[X]' if selected_flags[i] else '[ ]'} {options[i]}" for i in range(len(options))]
        render_menu(title, display_opts, sel, descs, footer="[W/S] Pohyb | [Space/D] Oznacit | [H] Vse | [N] Nic | [Enter] Potvrdit")
        k = read_key()

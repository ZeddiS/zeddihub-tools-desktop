...
1. `gui/updater.py` → `CURRENT_VERSION = "1.6.3"` (autoritativní pro runtime)
2. `gui/telemetry.py` → `_APP_VERSION = "1.6.0"` (⚠️ zaostává — synchronizovat!)
...
|---|---|---|---|
| TD-001 | Verze nesynchronizována: `updater.py` má `1.6.3`, `telemetry.py` má `1.6.0`, `version.json` root má `1.3.0` | gui/updater.py, gui/telemetry.py, version.json | Střední |
...
| `CURRENT_VERSION` | `"1.6.3"` | gui/updater.py |
...
*Soubor vygenerován: 2026-04-17 | Autor: Claude (Anthropic) | Na základě analýzy zdrojového kódu ZeddiHub Tools Desktop v1.6.x*

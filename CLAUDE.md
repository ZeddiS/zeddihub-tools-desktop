...
│   ├── version.py             # Single source of truth pro APP_VERSION (F-09, v1.9.0+)
│   ├── widgets.py             # Sdílené widget helpery (E-05, v1.9.0+): make_label/button/card/entry/...
...
```
gui/updater.py
  ├── CURRENT_VERSION = APP_VERSION  # importováno z gui/version.py (single source)
...
Verze aplikace má od **v1.9.0 single source of truth** v `gui/version.py`. Bumpování verze je úprava jediného souboru (web `version.json` přepisuje release bat).
1. `gui/version.py` → `APP_VERSION = "1.9.0"` (autoritativní runtime zdroj — F-09)
2. `gui/updater.py` → `from .version import APP_VERSION` (back-compat alias `CURRENT_VERSION = APP_VERSION`)
3. `gui/telemetry.py` → `from .version import APP_VERSION as _APP_VERSION`
4. `webhosting/data/version.json` → fallback pro offline klienty (přepsán release bat)
5. `version.json` (root) → informativní
...
|---|---|---|---|
| TD-001 | ~~Verze nesynchronizována~~ — kompletně vyřešeno ve v1.9.0 (F-09): `gui/version.py` je single source of truth. | gui/version.py, gui/updater.py, gui/telemetry.py | ✅ Vyřešeno |
| TD-002 | ~~Helper funkce duplicitně~~ — částečně vyřešeno ve v1.9.0 (E-05): nový `gui/widgets.py` poskytuje sjednocené `make_label`, `make_button`, `make_card`, `make_entry`, `make_stepper`, `make_dropdown`, `make_tabview`, `make_section_title`. Existující panely zatím nejsou migrovány — dokončit při dalším refactoringu. | gui/widgets.py, gui/panels/ | 🟡 Částečně vyřešeno |
...
| `APP_VERSION` | `"1.9.0"` | gui/version.py (single source of truth) |
| `CURRENT_VERSION` | `= APP_VERSION` | gui/updater.py (back-compat alias) |
...
*Soubor aktualizován: 2026-04-19 | Autor: Claude (Anthropic) | Na základě analýzy zdrojového kódu ZeddiHub Tools Desktop v1.9.0*
